...
python3 main.py
```
Al abrir la aplicación se mostrara una pantalla donde veremos dos botones, el del primer parcial; donde se pueden encontrar todas las practicas anteriores a esta con los temas de Automatas Finitos y Expresiones Regulares. De ahi tenemos el otro boton, que muestra las practicas de segundo parcial con los temas de Automatas de Pila y Gramaticas libres de contexto que es de lo que habla esta practica asi que elegiremos este boton

Al presionar el boton de segundo parcial se abriran todas las pestañas correspondientes a las practicas de este parcial, la primer pestaña "Simulador AP" nos ayudara a cargar un Automata de pila apartir de un archivo de la aplicación JFLAP (.jff). Al oprimir el boton "Cargar .JFF (AP)" se abrira una pestaña de archivos para que el usuario elija el archivo .JFF donde se encuentra el Automata de Pila, al seleccionar esta archivo, se mostrara en la pantalla de la aplicación el mismo diagrama que se tiene en el archivo .JFF además de la 7-tupla de nuestro Automata de Pila para completarlo. 
Por la parte abajo de la pestaña se encuentra una caja de texto donde se puede probar si una cadena es aceptada o rechazada por el Automata de Pila. En esta caja de texto se introduce la cadena y al apretar elboton "Validar con Pila" Nos muestra todo el recorrido de la cadena en el Automata, en cada paso se muestra el caracter que se lee de la cadena, el tope de la pila (pop) y lo que entra en la pila (push), por ultimo se muestra el estado de la cadena que puede ser; "Aceptada" o "Rechazada"

La segunda pestaña de segundo parcial "Constructor AP" nos ayudara, como dice su nombre, a construir desde cero el Automata de Pila. Para empezar el usuario debera llenar la 7-tupla que conforma el Automata; Estados de este, el alfabeto, el alfabeto de la pila, el estado inicial y el o estados finales. Despues se debera llenar la tabla de transiciones del automata, se espesifica como poner todos los datos: El estado en el que esta, el caracter que se lee en la cadena, lo que se encuentra en el tope de la pila, el estado de destino y lo que se introduce a la pila. Al tener todos los datos del automata se pueden hacer dos cosas, construirlo y graficar o guardarlo en el formato de JFLAP, al presionar el primer boton se valida que los datos introducidos son correctos y se grafica el Automata de Pila. El siguiente boton nos ayuda a guardar el automata en un archivo .JFF abriendo la pestaña de archivos del usuario para que lo guarde como guste. En la parte de abajo se encuentra de igual forma como en la pestaña anterior una caja de texto donde se puede probar si una cadena es aceptada o rechazada por el Automata de Pila. En esta caja de texto se introduce la cadena y al apretar elboton "Validar Cadena" Nos muestra todo el recorrido de la cadena en el Automata, en cada paso se muestra el caracter que se lee de la cadena, el tope de la pila (pop) y lo que entra en la pila (push), por ultimo se muestra el estado de la cadena que puede ser; "Aceptada" o "Rechazada"

En la ultima pestaña de nuestro software "Gramaticas LLC" nos ayudara a construir, como lo dice su nombre, Gramaticas LIbres de Contexto, para esto el usuario solo necesita tener las reglas de producción de la gramatica, en esta pestaña se tienen que meter todas las reglas de producción en la caja de texto debajo del texto "Producciones", al tener todas las reglas de produccion se presiona el boton "Cargar Gramatica" para crearla, se le notificara al usuario si la gramatica estaba cargada correctaente, despues en la parte de abajo se encuentra otra caja de texto que indica al usuario para introducir una cadena para probar si era valida en esa gramatica. Al presionar el boton "Validar Derivación" se muestra primero el estado de la cadena, si esta es aceptada o rechazada, despues, en la parte de abajo se mostrara toda la ruta de derivación completa con esto el usuario podra corroborar que la cadena es aceptada o rechazada
