...
# NUEVO: Lógica del Segundo Parcial
...
class Launcher:
    def __init__(self, root):
        ...
        tk.Label(self.root, text="SELECCIONA EL ENTORNO DE TRABAJO", bg="#0f172a", fg="white", font=("Arial", 16, "bold")).pack(pady=40)
        
        tk.Button(self.root, text="PRIMER PARCIAL\n(Autómatas Finitos y Expresiones Regulares)", 
                  command=self.lanzar_p1, bg="#059669", fg="white", font=("Arial", 12, "bold"), pady=20).pack(fill="x", padx=100, pady=10)
                  
        tk.Button(self.root, text="SEGUNDO PARCIAL\n(Autómatas de Pila y Gramáticas)", 
                  command=self.lanzar_p2, bg="#2563eb", fg="white", font=("Arial", 12, "bold"), pady=20).pack(fill="x", padx=100, pady=10)

    def lanzar_p1(self):
        self.root.destroy()
        new_root = tk.Tk()
        app = App(new_root) # Llama a tu clase App original
        new_root.mainloop()
